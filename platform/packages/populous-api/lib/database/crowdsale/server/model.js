  ...
  meteorMethods: {
    ...
    async closeAuctionsByWinnderBid(winnerBid, invoice, forceInsert = false, isAcceptOffer = false) {
      ...
      if (winnerBid && invoice) {
          if (invoice.status !== invoiceStatuses.auctionOpen) {
            throw new Meteor.Error(403, 'Auction is not in open status');
          }
          let soldPrice = 0;
          ...
          const transferFundsToPopulous = ({userId, amount}) => {
            const bidderWallet = Wallet.findOne({userId: userId});
            const operationFeeAmount = bidderWallet.getOperationFee(false);
            const pptAmount = bidderWallet.convertCurrencyToPPT(invoice.currency, amount, false)
            new BlockchainAction({
              userId,
              title: 'PPT',
              type: blockchainActionTypes.withdraw,
              amount: pptAmount * 0.25 * 0.01, // 0.25 % of the bid amount
              feeAmount: operationFeeAmount,
              status: blockchainActionStatuses.pending,
              to: process.env.ADMIN_EXTERNAL_WALLET,
              version: bidderWallet.version,
              data: {
                invoiceId: invoice._id
              }
            }).save()
          ...
          applyFunctionForEachWinner(transferFundsToPopulous);
