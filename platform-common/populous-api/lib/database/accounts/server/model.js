    ...
    async sendToVerifyKYCDocuments() {
      ...
      if (this.companyNumber && this.country === 'GB') {
        try {
          ...
          this.companyDescription = sicCodes.map(code => sicList.find(item => item.SIC === code) ? sicList.find(item => item.SIC === code).description : '').join();
        } catch (error) {
          throw new Meteor.Error(400, 'Company doesn\'t exist.');
        }
      }
