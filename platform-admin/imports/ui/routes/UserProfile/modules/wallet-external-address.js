...
import { requires2FA } from '../../../../ui/components/Requires2FA/modules/actions';
import { toggleWalletAddressEdit, toggleAddWalletAddress } from './actions';
  ...
  return (dispatch) => {
    ...
    new ExternalAddress().callMethod('addAddress', address, name, (err, result) => {
      if (err) {
        ...
        toastr.success('Success', 'Wallet address has been added');
      }
    });
  };
};
...
let editWalletAddress = (externalAddress, newAddress) => {
  return (dispatch) => {
    if (!externalAddress) {
      return;
    }
    ...
    externalAddress.callMethod('editAddress', newAddress, (err, result) => {
      if (err) {
        toastr.error('Error', err.reason);
      } else {
        dispatch(toggleWalletAddressEdit(false));
        toastr.success('Success', 'Wallet address has been changed');
      }
    });
  };
};
  ...
  return (dispatch) => {
    ...
    externalAddress.callMethod('removeAddress', (err, result) => {
      if (err) {
        ...
        toastr.success('Success', 'Wallet address has been removed');
      }
    });
  };
};
