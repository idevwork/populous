...
Wallet.extend({
  meteorMethods: {
    async getBalanceError() {
      ...
      let walletBalance = this.balance;

      if (walletBalance < totalFromValue) {
        throw new Meteor.Error(400, 'Unfortunately your wallet balance is less than total fromValue.');
      } else {
        // SECTION 2 - ignore this for now
        // if the users toValue is less than ledgerAmount. Talk about invoices.
        let ledgerBalances = await LedgerBalance.find({ userId }).fetch();
        for (let i = 0; i < ledgerBalances.length; i++) {
          await ledgerBalances[i].getBalance()
        }
      }
    },
    ...
    async calculateAvailableBalanceUSDC(balanceUSDC, shouldSave = true) {
      this.checkIsActive();
      let availableBalanceUSDC = balanceUSDC;

      if (!availableBalanceUSDC) {
        availableBalanceUSDC = this.balanceUSDC;
      }

      availableBalanceUSDC -= await this.getInCollateralUSDC();

      availableBalanceUSDC -= await BlockchainAction
        .find({
          userId: this.userId,
          type: blockchainActionTypes.withdraw,
          title: 'USDC',
      ...
      if (shouldSave) {
        this.availableBalanceUSDC = availableBalanceUSDC;
        await this.save()
      }

      return availableBalanceUSDC;
    },

    async getInCollateral(type = depositLogTypes.PPT) {
    ...
    async getInCollateralUSDC() {
      const pendingBlockchainActions = await BlockchainAction.find({
        userId: this.userId,
        type: blockchainActionTypes.usdcToUsdp,
        status: blockchainActionStatuses.pending,
      });

      const pendingLedgerLogIds = pendingBlockchainActions.map(pl => pl.data.ledgerLogId)
      const depositLogs = await DepositLog.find({
        returned: false,
        logId: {$in: pendingLedgerLogIds}
      });
    ...
    async depositToCurrency(currencySymbol, pptAmountInput) {
      ...
      const resultAmount = this.convertPptToCurrency(pptAmount, currencySymbol, true);
    ...
    async depositUSDCToGBPp(currencySymbol, amountInput) {
      this.checkIsActive();

      const userId = checkAuth();
      const amount = roundNumber(amountInput, 8);
      if (!currencySymbol) {
        throw new Meteor.Error(403, 'Currency is required');
      }
      ...
      const availableBalance = this.calculateAvailableBalanceUSDC();

      if (typeof amount !== 'number' || amount <= 0 || amount > availableBalance) {
        throw new Meteor.Error(403, 'Deposit amount should not be more then available balance and valid positive number');
      }
      ...
      const resultAmount = this.convertUSDCToCurrency(amount, currencySymbol, false);
      const conversionRate = roundNumber(1 / (resultAmount / amount), 8);
      ...
      const blockchainAction = new BlockchainAction({
        ...
        amount: amount,
        status: blockchainActionStatuses.pending,
        data: { resultAmount, ledgerLogId: ledgerLog._id, depositLogId: depositLog._id },
        version: this.version,
      });

      await blockchainAction.save();

      this.availableBalanceUSDC = this.availableBalanceUSDC - amount;
      await this.save();
    },
    ...
    async withdrawPPT(amountInput, toAddressId, dataId) {
      ...
      const pendingBidsPPT = await this.calculatePendingBidsPPTAmount(userId) * 0.25 * 0.01;

      if (!isAdminTrigger && (amount + operationFeeAmount) > (currentBalance - pendingBidsPPT)) {
        throw new Meteor.Error(400, 'Typed amount above available amount');
      }
    ...
    async pptToUsdt(amountPPT, needDivide = true) {
      try {
        return roundNumber(amountPPT * (await this.getLatestPptToUsdtRate() * (needDivide ? 0.01 : 1)));
      } catch (e) {
    ...
    async convertUSDCToCurrency(amount, toCurrency, needDivide = true) {
      const usdtAmount = roundNumber(amount);
      let exchangeRate = 1;
      ...
      if (toCurrency !== 'USD') {
        ...
        exchangeRate = exchangeRateDocument.ask - 0.025;
      }
    ...
    async calculatePendingBidsPPTAmount(userId) {
      let openInvoices = Invoice.find(
        {status:invoiceStatuses.auctionOpen},
        {_id: 1}
      ).fetch();
      let openInvoiceIds = openInvoices.map(invoice => invoice._id);
      ...
      for (let i = 0; i < userBids.length; i++) {
        const userBid = userBids[i]
        let invoice = invoices.find(invoice => invoice._id === userBid.invoiceId)
        let bidAmount = userBid.amount
        if (userBid.isGroup()) {
          bidAmount = userBid.bidders.find(bidder => bidder.userId === userId).amount
        }
        const amount = await this.convertCurrencyToPPT(invoice.currency, bidAmount)
        pptAmount += amount
      }
      return pptAmount
    },
